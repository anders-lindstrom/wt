...
func TestWayOutCoversEverySituation(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"E: aborted by hand", Way{Plan: true, Aborted: true},
			[]string{"aborted", undo + " bump clears its plan", run + " bump"}, []string{resume, force, abort}},
		{"E without a name", Way{Plan: true, Aborted: true, Work: "", Path: ""},
			[]string{"aborted", undo + " clears its plan", run + " starts again"}, []string{"bump"}},
		{"K: undo, then run again", Way{Plan: true, Rebasing: true, Restart: true},
			[]string{undo + " bump", run + " bump"}, []string{resume, force, abort, "aborted", "finished"}},
