...
check() {
    ...
    # The user settings are this case's own: `wt config set` writes a real
    # file, and the developer's must never be the one an example edits.
...
check worktrees myrepo_wt/fix_wt/login-crash "" 'wt adopt . --no-build'
...
check worktrees myrepo_wt/fix_wt/login-crash "" 'wt setup --no-build'
...
check fleet "" "" 'wt repos --all --paths'
check fleet "" "" 'wt list --all --no-pr'
check fleet "" "" 'wt list --roots work --refresh'
check fleet "" "" 'wt list --profile api'
check fleet "" "" 'wt status --all | grep behind'
check fleet "" "" 'wt status --roots work'
check fleet "" "" 'wt status --profile api'
check fleet "" "" 'wt doctor --all'
check fleet "" "" 'wt doctor --roots work'
check fleet "" "" 'wt doctor --profile api'
...
check gh "" "" 'wt pr list'
check gh "" "" 'wt pr ls'
check gh "" "" 'wt pr list | cat'
check gh "" "" 'wt pr checkout' fail
check gh "" "" 'wt pr checkout 12'
check gh "" "" 'wt pr co 12'
check gh "" "" 'wt pr checkout 12 --no-setup'
check gh "" "" 'wt pr checkout 12 --skip-build'
check gh "" "" 'wt pr checkout 12 --no-build'
...
check plain "" "" 'wt config get branch_suffix'
check plain "" "" 'wt config set branch_suffix ""'
check plain "" "" 'wt config unset branch_suffix'
check plain "" "" 'wt config get type_names'
check plain "" "" 'wt config set type_names "feat=feature docs=doc"'
check plain "" "" 'wt config unset type_names'
...
check plain "" "" 'wt schema status > wt-status.schema.json'
check plain "" "" 'wt version'

check sync myrepo_wt/fix_wt/login-crash "" 'wt up'
check sync myrepo_wt/fix_wt/login-crash "" 'wt up --push'
check sync "" "" 'wt up login-crash'
check sync myrepo_wt/fix_wt/login-crash "" 'wt up --no-fetch --yes'
check sync myrepo_wt/fix_wt/login-crash "" 'wt up --no-push'
check sync myrepo_wt/fix_wt/login-crash "" 'wt up --yes --no-push --json'
check sync myrepo_wt/fix_wt/login-crash "" 'wt up --json --expect 1:0123abcd --no-fetch' fail
...
check keep "" "" 'wt sync keep once'
check keep "" "" 'wt sync keep status'
check keep "" "" 'wt sync keep once --no-push'
